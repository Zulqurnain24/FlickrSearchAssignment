...
extension FlickrSearchViewController {
    ...
    @objc func postNotification( _ completionHander: (() -> Void)? = nil) {
        let flickrPhoto = photoForIndexPath(indexPath: selectedIndexPath!)
        NotificationCenter.default.post(name: NSNotification.Name(Constants.imageDetailNotificationName.rawValue), object: flickrPhoto)
    ...
    // MARK: - Helper Method
    internal func loadMorePhotos(_ completionHandler: (() -> Void)? = nil) {
        guard let searchText = self.searchTextField.text, searchText.count > 0 else {
            return
        }
        if !loadMore && paging!.currentPage! < paging!.totalPages! {
            loadMore = true
            self.callSearchApi(searchText: searchText, pageNo: paging!.currentPage! + 1, completionHandler)
        }
    }
}
